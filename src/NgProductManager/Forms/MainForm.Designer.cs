...
partial class MainForm
{
    ...
    private void InitializeComponent()
    {
        ...
        buttonNewCase = AddButton(actions, "新規NG登録", 120); buttonShowDetail = AddButton(actions, "詳細表示", 100); buttonReinspection = AddButton(actions, "再検査登録", 110); buttonCloseCase = AddButton(actions, "検査OK・クローズ", 130); buttonMaster = AddButton(actions, "マスター管理", 110); buttonCsv = AddButton(actions, "CSV出力", 90); buttonReload = AddButton(actions, "再読込", 90); buttonBackup = AddButton(actions, "バックアップ", 100); buttonRestore = AddButton(actions, "復元", 80);
